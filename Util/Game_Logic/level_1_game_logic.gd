...
func restart_game() -> void:
    ...
    # Reset players' wave_completed status
...
func all_players_completed_wave() -> bool:
    # Check if all players have completed the current wave
    for peer_id in players.keys():
        if not players[peer_id].get("wave_completed", false):
            return false
    return true
...
func star_touched(player_name) -> void: # This function is called when a star is touched by any peer
    ...
    if multiplayer.is_server(): # only the server should delete the player
        players[peer_id]["wave_completed"] = true # Update the player's wave_completed status
...
func start_first_wave() -> void:
    ...
    print("game_logic.gd - start_first_wave() - Initializing wave system")
...
func start_wave(wave_number: int) -> void:
    ...
    print("game_logic.gd - start_wave() - Starting wave %d" % wave_number)
    ...
    # Reset all players' wave completion status
    for peer_id in players.keys():
        players[peer_id]["wave_completed"] = false
...
func spawn_enemies(enemy_types: Array, count: int) -> void:
    if not multiplayer.is_server():
        return
    ...
    for i in range(count):
        ...
        # Set random spawn position (avoiding the center where players might be)
        ...
        var center = Vector2(arena_dimension_x[0] + (arena_dimension_x[1] - arena_dimension_x[0]) / 2, arena_dimension_y[0] + (arena_dimension_y[1] - arena_dimension_y[0]) / 2)
        var min_distance = 150
        while spawn_position.distance_to(center) < min_distance:
